...
import { verifyPaymentSession } from "../api/client";
...
export default function OrderSuccess() {
  ...
  useEffect(() => {
    ...
    async function verify() {
      try {
        ...
        if (isMounted) {
          setOrder({ id: orderId, status: "Paid", amount: response.payment?.amount });
          setVerifying(false);
        }
      } catch (err) {
        if (isMounted) {
    ...
    return (
      <div className="max-w-xl mx-auto px-4 pt-40 pb-24 text-center">
        <div className="flex flex-col items-center justify-center gap-4">
          ...
          <p className="tracked text-xs text-ink-soft">Verifying your payment...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="max-w-xl mx-auto px-4 pt-40 pb-24 text-center">
        ...
        <p className="tracked text-xs text-ink mb-2">Payment Verification Failed</p>
  ...
  return (
    <div className="max-w-xl mx-auto px-4 pt-40 pb-24 text-center pop-in">
      ...
      <p className="tracked text-xs text-ink-soft mb-2">Order Confirmed & Paid</p>
      ...
      {order?.amount && (
        <p className="text-ink mb-6 text-sm font-medium">Amount Paid: {formatPKR(order.amount)}</p>
      )}
